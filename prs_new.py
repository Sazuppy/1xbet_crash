...
def prs():
    ...
    time.sleep(30)
    # сортировка по WS
    # mouse.click(button='left', coords=(1326, 224))
    # time.sleep(5)
    # mouse.click(button='left', coords=(1346, 459))
    # time.sleep(5)
    # переход в окно websocket наименование crash
    mouse.click(button='left', coords=(1178, 405))
    time.sleep(5)
    ...
    while not stop_pressed:
        ...
        time.sleep(1)
        mouse.click(button='left', coords=(1254, 478))
        mouse.press(button='left', coords=(1254, 478))
        # клик по сообщениям websocket
        # time.sleep(1)
        mouse.release(button='left', coords=(1693, 687))
        # keyboard.send_keys('^a')
        # копирование в буфер
        keyboard.send_keys('^c')
        # сброс выделения
        time.sleep(1)
        mouse.click(button='left', coords=(1154, 719))
        ...
        if count_r >= 30:
            ...
            time.sleep(30)
            # # сортировка по WS
            # mouse.click(button='left', coords=(1260, 195))
            # time.sleep(5)
            # mouse.click(button='left', coords=(1257, 432))
            # time.sleep(5)
            # переход в окно websocket наименование crash
            mouse.click(button='left', coords=(1178, 405))
            time.sleep(5)
        ...
        for i in range(0,len(value_pr),3):
            ...
            if 'target' in value_json:    
                ...
                if value_json['target'] == 'OnStart':
                    
                    with open(file_name, 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow(list_value)  
                print(list_value)        
                df = pd.read_csv('my_file_new.csv')
                df.tail(100).to_csv('my_file_new.csv', index=False)
